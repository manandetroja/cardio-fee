...
export default function Insights() {
  ...
  return (
    <PageTransition className="container max-w-5xl mx-auto px-4 py-8">
      ...
      >
        <div className="inline-block p-4 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 mb-6 shadow-[0_0_20px_rgba(99,102,241,0.2)]">
          <Brain className="w-12 h-12 text-indigo-400" />
        </div>
        <h1 className="text-4xl font-bold text-white mb-4 font-['Plus_Jakarta_Sans']">Model Intelligence</h1>
      ...
      <motion.div
        ...
        className="glass-panel p-8 md:p-10 border border-white/5"
      >
        <div className="flex items-center gap-3 mb-8 border-b border-white/5 pb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Benchmark Performance</h2>
            <p className="text-slate-400 text-sm">Comparative accuracy across tested algorithms</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {sortedModels.map((item, index) => (
            <div key={index} className="flex flex-col sm:flex-row sm:items-center gap-3 group">
              <span className={`w-48 text-sm font-semibold ${item.production ? 'text-indigo-400 font-bold' : 'text-slate-400'}`}>
                {item.model}
              </span>

              <div className="flex-1 h-10 bg-slate-900/50 rounded-lg overflow-hidden relative border border-white/5">
                <motion.div
                  initial={{ width: 0 }}
                ...
                {item.production && (
                  <div className="absolute top-0 right-0 h-full flex items-center pr-3">
                    <span className="text-[10px] uppercase font-bold text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded shadow-sm flex items-center gap-1">
...
function InsightCard({ icon, title, description, delay }) {
  return (
    <motion.div
      ...
      className="glass-panel p-6 rounded-2xl border border-white/5 hover:border-indigo-500/30 transition-all hover:-translate-y-1 hover:bg-white/5"
    >
      <div className="mb-4 p-3 bg-white/5 rounded-xl inline-block border border-white/5">{icon}</div>
      <h3 className="text-lg font-bold text-white mb-2">{title}</h3>
      <p className="text-slate-400 leading-relaxed text-sm h-20">{description}</p>
    </motion.div>
  );
}
